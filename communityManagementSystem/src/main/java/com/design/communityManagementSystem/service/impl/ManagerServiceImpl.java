...
import com.design.communityManagementSystem.service.ManagerService;
...
public class ManagerServiceImpl implements ManagerService {
    ...
    public RestFulBean<String> updatePassword(Users users) {
        ...
        if(users.getRole().equals("用户")){
            Users usered=this.usersMapper.queryById(users.getId());
            if(!users.getOldPassword().equals(usered.getPassword())){
                return RestFulBean.error("旧密码错误");
            }
            usered.setPassword(users.getNewsPassword());
            this.usersMapper.update(usered);//执行数据库的修改语句 根据id 修改
        }
        return RestFulBean.succ("修改成功");
    }
}
